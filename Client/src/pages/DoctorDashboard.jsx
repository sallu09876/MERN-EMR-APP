...
const StatCard = ({ icon, label, value, color, delay=0 }) => (
  <div style={{
    ...
    <div style={{ width:"48px",height:"48px",borderRadius:"12px",background:`${color}18`,
      display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.4rem",flexShrink:0 }}>{icon}</div>
    <div>
...
export const DoctorDashboard = () => {
  ...
  return (
    <div>
      ...
      <div style={{ background:"white",borderRadius:"14px",padding:"1.25rem 1.5rem",marginBottom:"1.25rem",boxShadow:"var(--shadow-sm)",border:"1px solid var(--border)",display:"flex",flexWrap:"wrap",gap:"0.75rem",alignItems:"flex-end",animation:"fadeUp 0.4s ease 0.18s both" }}>
        <div>
        ...
        <div style={{ marginLeft:"auto",fontSize:"0.8rem",color:"var(--text-muted)",alignSelf:"center" }}>{pagination.total} appointment{pagination.total!==1?"s":""}</div>
      </div>
      ...
      {loading ? <Loader text="Loading appointments…" /> : (
        <div style={{ background:"white",borderRadius:"14px",boxShadow:"var(--shadow-sm)",border:"1px solid var(--border)",overflow:"hidden",animation:"fadeUp 0.4s ease 0.22s both" }}>
          ...
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0.875rem 1.5rem",borderTop:"1px solid var(--border)",background:"var(--surface-2)" }}>
